...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-04-18/70d1a982-e517-4799-a083-60678921587c/dfe5663c50e87a34b3b92da13be063d6e9d22966f39a27501588ad4ff59666d2.jpg)
...
数据流转过程如下：用户在前端填写表单 $\rightarrow$ 后端验证数据 $\rightarrow$ 调用智能体系统 $\rightarrow$ 智能体依次调用景点搜索、天气查询、酒店推荐、行程规划 Agent $\rightarrow$ 每个 Agent 通过 MCP 协议调用外部 API $\rightarrow$ 整合结果返回前端 $\rightarrow$ 前端渲染展示。
...
- 高德地图 Web 服务 Key: 访问 https://console.ameda.com/ 注册并创建应用
...
1. 进入后端目录
cd helloagents-trip-planner/trunk
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-04-18/70d1a982-e517-4799-a083-60678921587c/73fa20d8cbfc0ddbe8f1224afa3c0d5800eaca39d3aa9fc89f79d5150573a707.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-04-18/70d1a982-e517-4799-a083-60678921587c/a3311005391514ea780fd8ea19f066d717bcf912e8cc24698d077715e5220f5d.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-04-18/70d1a982-e517-4799-a083-60678921587c/e8b401436879e6f177d8e9226c494c63c70657405d2a1ce6fdc17469de45de0c.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-04-18/70d1a982-e517-4799-a083-60678921587c/3af2bcf46f475e7b2cf9c22e1d960e39996f21eb6ca23525fd1b445467defc2d.jpg)
...
在这个过程中，数据经历了多次转换：前端表单 $\rightarrow$ HTTP 请求 $\rightarrow$ 后端 Python 对象 $\rightarrow$ 外部 API 响应 $\rightarrow$ 后端 Python 对象 $\rightarrow$ HTTP 响应 $\rightarrow$ 前端 TypeScript 对象 $\rightarrow$ 页面展示。如果没有统一的数据格式，每一步转换都可能出错。这就是为什么我们需要数据模型。
...
attraction $=$ { "name":"故宫", "location": {"lng":116.397128,"lat":39.916527}, "price":60 } # 访问数据 lng $=$ attraction["location】【lng"]
...
class Location(BaseModel): longitude: float $=$ Field(...,description $\equiv$ "经度") latitude: float $=$ Field(...,description $\equiv$ "纬度")   
class Attraction(BaseModel): name: str location: Location ticket_price:int $= 0$ # 创建对象   
attraction $=$ Attraction( name $\equiv$ "故宫", location $\equiv$ Location(longitude $= 116.397128$ ,latitude $= 39.916527)$ ticket_price $= 60$ ）   
# 类型安全的访问   
...
from typingimportOptional,List   
classAttraction(BaseModel): name:str $=$ Field(...,description $\equiv$ "景点名称") #必填 rating：float $=$ Field(default=0.0,ge=0,1e=5)#默认值，范围验证 visit_duration:int $=$ Field(default=60,gt=0)#大于0 description:Optional[str] $\equiv$ None #可选字段
...
class weatherInfo(BaseModel): temperature:int @field_validator('temperature',mode $\equiv$ 'before') def parse_temperature(cls,v): ""解析温度字符串：" $16^{\circ}C$ 一 $\rightharpoondown$ 16"" ifisinstance(v,str): v=v.replace('c','').replace('C'，').strip() return int(v) return v
...
这里我们使用了范围验证(ge表示大于等于，1e表示小于等于)，确保经纬度的值在合理范围内。
...
class Budget(BaseModel):
    ""预算信息''
    total(attractions: int = Field(default=0, description="景点门票总费用")
    total_hoteIs: int = Field(default=0, description="酒店总费用")
    total.meals: int = Field(default=0, description="餐饮总费用")
...
class DayPlan(BaseModel):
    ...
    meals: List[Meal] = Field(defaultFactory=list, description="餐饮安排")
```

注意这里使用了List[Attraction]来表示景点列表，defaultFACTORY=list表示默认值是一个空列表。

天气信息需要特殊处理，因为高德地图返回的温度格式不规范。我们使用自定义验证器来处理：

```python
class WeatherInfo(BaseModel):
    '''天气信息''' date: str = Field(..., description="日期") day/weather: str = Field(..., description="白天天气") night/weather: str = Field(..., description="夜间天气") day_temp: int = Field(..., description="白天温度(摄氏度)") night_temp: int = Field(..., description="夜间温度(摄氏度)") windDirection: str = Field(..., description="风向") wind_power: str = Field(..., description="风力") @field_validator('day_temp', 'night_temp', mode='before') def parse_temperature(cls, v):
    '''解析温度字符串："16°C" -> 16''' if isinstance(v, str):
        v = v.replace('°C', '') . replace('°C', '') . replace('°', '') . strip()
        return int(v)
    except ValueError:
```
...
class TripPlan(BaseModel):
    ...
    days: List[DayPlan] = Field(defaultFACTORY=list, description="每日行程")
    weather_info: List[WeatherInfo] = Field(defaultFACTORY=list, description="天气信息")
    overall Suggestions: str = Field(..., description="总体建议")
...
app $=$ FastAPI()   
@app.post("/api/trip/plan",response_model $\equiv$ TripPlan)   
async def create_trip計劃(request:TripPlanRequest) -> TripPlan: 创建旅行计划   
FastAPI自动： 1.验证请求数据(TripPlanRequest) 2.验证响应数据(TripPlan) 3.生成OpenAPI文档 trip計劃 $=$ await generate_trip計劃(request) return trip計劃
...
2. 使用maps/weather查询天气，获取未来几天的天气预报
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-04-18/70d1a982-e517-4799-a083-60678921587c/481ca357746156f123a0c174b2adb36b632064df006f2665ce6fccca46edd24e.jpg)
...
```txt
ATTRACTION(agent_prompt = '''你是景点搜索专家。
```
...
```txt
`[TOOL_CALL:amap_maps/weather:city=城市名]`
```
...
class TripPlannerAgent: def__init__(self): self.attraction_agent $=$ SimpleAgent(name $\equiv$ "景点搜索"prompt $\equiv$ ATTRACTION_PROMPT) self.weather_agent $=$ SimpleAgent(name $\equiv$ "天气查询",prompt $\equiv$ WEATHER_PROMPT) selfhotel_agent $=$ SimpleAgent(name $\equiv$ "酒店推荐",prompt $\equiv$ HOTEL_PROMPT) self.planner_agent $=$ SimpleAgent(name $\equiv$ "行程规划",prompt $\equiv$ PLANNER.promPT) defplan_trip(self, request:TripPlanRequest）->TripPlan: #步骤1：景点搜索 attraction_response $=$ self.attraction_agent.run( f"请搜索{request.city}的{requestpreferences}景点") #步骤2：天气查询 weather_response $=$ selfweather_agent.run( f"请查询{request.city}的天气"

步骤3：酒店推荐 hotel_response = selfhotel_agent.run(
...
#步骤5：解析JSON   
trip計劃 $=$ self._parse_trip計劃(planner_response)   
return trip計劃

这个流程顺序执行四个步骤，每个步骤的输出作为下一个步骤的输入。注意我们使用了TripPlanRequest和TripPlan这两个Pydantic模型，这是在13.2节中定义的。
...
```txt
def_build_planner_query( self, request:TripPlanRequest, attraction_response: str, weather_response: str, hotel_response: str) -> str:"""构建规划Agent的查询""return f""请根据以下信息生成{request.city}的{request.day}日旅行计划：**用户需求：**- 目的地：{request.city} - 日期：{request.start_date}至{request.end_date} - 天数：{request(days]天- 偏好：{request/preferences} - 预算：{request.budget} - 交通方式：{request.transportation} - 住宿类型：{request.accommodation}**景点信息：**{attraction_response}**天气信息：**{weather_response}**酒店信息：**{hotel_response}请生成详细的旅行计划，包括每天的景点安排、餐饮推荐、住宿信息和预算
```
...
import requests   
def search poi(keywords: str, city: str, api_key: str): ""直接调用高德地图POI搜索API"" URL $=$ "https://restapi.ameda.com/v3/place/text" params $=$ { "keywords": keywords, "city": city, "key": api_key, "output": "json" } response $=$ requests.get(url, params=params) data $=$ response.json() return data

这种方式看起来很简单，但在实际使用中会遇到几个问题。首先是Agent无法自主调用。在我们的HelloAgents框架中，Agent通过识别提示词中的工具调用标记(比如

[TOOL_CALL:tool_name:arg1=value1)]来调用工具。如果我们直接在代码中调用API，Agent就失去了自主决策的能力，变成了一个简单的函数调用。
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-04-18/70d1a982-e517-4799-a083-60678921587c/91b444fd6b9cf1a01bab291ad354299784c757aa40630af910be579cb1874017.jpg)



图13.7 amap-mcp-server工具
...
<table><tr><td>类别</td><td>工具</td><td>功能</td></tr><tr><td rowspan="3">POI搜索</td><td>amap_maps_text_search</td><td>文本搜索POI</td></tr><tr><td>amap_maps_search_detail</td><td>获取POI详情</td></tr><tr><td>amap_maps_around_search</td><td>周边搜索</td></tr><tr><td rowspan="3">路线规划</td><td>amap_maps_directionnement.walk-by_address</td><td>步行路线</td></tr><tr><td>amap_maps_directionnement-driving-by-address</td><td>驾车路线</td></tr><tr><td>amap_maps_directionnement_transit_integrated-by-address</td><td>公交路线</td></tr><tr><td>天气查询</td><td>amap_maps/weather</td><td>天气查询</td></tr><tr><td rowspan="2">地理编码</td><td>amap_maps_geocode</td><td>地址→坐标</td></tr><tr><td>amap_maps_regeocode</td><td>坐标→地址</td></tr></table>
...
# 创建MCP工具   
mpc_tool $=$ MCPTool( name $\coloneqq$ "amap_mcp", command $\coloneqq$ "npx", args $\coloneqq$ ["-y","@sugarforever/amap-mcp-server"], env $\equiv$ {"AMAP_API_KEY": settings. amap api key}, auto Expand $\equiv$ True   
）

这段代码做了什么呢？首先，command 和 args 指定了如何启动 MCP 服务器。npx -y

@sugarforever/amap-mcp-server会从npm仓库下载并运行amap-mcp-server这个包。env参数传递了环境变量，这里我们传递了高德地图的API密钥。
...
最关键的是 auto Expand=True 这个参数。当设置为 True 时，MCPTool 会自动查询 MCP 服务器提供了哪些工具，然后为每个工具创建一个独立的 Tool 对象。这就是为什么我们只创建了一个 MCPTool，但 Agent 却获得了 16 个工具。让我们看看这个过程：

创建一个MCPTool  
mpc_tool $=$ MCPTool(.,auto Expand $\equiv$ True)agent.add_tool(mcp_tool)  
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-04-18/70d1a982-e517-4799-a083-60678921587c/d7f585475d934266ae4632b4584b2767b6df925add12836ced1b275405cad2f4.jpg)
...
class TripPlannerAgent: def__init__(self): settings $=$ get_settings() self.llm $=$ HelloAgentsLLM() #创建共享的MCP工具实例(只创建一次） self.mcp_tool $\equiv$ MCPTool( name $\coloneqq$ "amap_mcp", command $\coloneqq$ "npx", args $\coloneqq$ ["-y","@sugarforever/amap-mcp-server"], env $\equiv$ {"AMAP_API_KEY":settings.amlmkey}, auto Expand $\equiv$ True ） #创建多个Agent，共享同一个MCP工具 self.attraction_agent $=$ SimpleAgent( name $\coloneqq$ "AttractionSearchAgent", llm $\equiv$ self.llm,
...
selfhotel_agent = SimpleAgent(  
name="HotelAgent",  
11m= self.11m,  
system_prompt=HOTEL_AGENT_promPT  
)  
selfhotel_agent.add_tool(self.mcp_tool) # 共享
```
...
```python
# backend/app/services/unsplash.service.py
import requests
from typing import Optional, List, Dict
import logging
logger = logging.getLogger(_name_)
class UnsplashService:
    '''Unsplash图片服务''' def __init__(self, access_key: str):
        self.access_key = access_key
        self.base_url = "https://api.unsplash.com"
def searchphotos(self, query: str, per_page: int = 10) -> List[Dict]:
    '''搜索图片''' try:
...
```python
"per_page": per_page, "client_id": self.access_key
}
...
for result in results:
    photos.append({
        ...
        "description": result.get("description", "", "photographer": result["user"},{"name}]
    })
```

这个服务类提供了两个方法：searchphotos搜索多张图片，getphoto_url获取单张图片的URL。我们在API路由中使用这个服务，为每个景点获取图片：

```python
# backend/app/api/routes/trip.py
from app/services.Unsplashservice import UnsplashService
unsplash.service = UnsplashService(settings.unsplash_access_key)
@router.post("/plan", response_model=Triplan)
async def create_trip計劃(request: TripPlanRequest) -> TripPlan:
    # 生成旅行计划
    trip計劃 = trip_planner(agent规划设计)(request)
    # 为每个景点获取图片
    for day in trip計劃的日子里:
        for attraction in day.atTRACTIONS:
            if not attraction(image_url:
                image_url = Unsplash_service.get.Photo_url(f"[attraction.name]{trip計劃.city}")\n\nattraction(image_url = image_url
```
...
在我们的智能旅行助手项目中，后端是用Python和FastAPI实现的，提供了一个核心API接口POST /api/trip/plan，接收旅行需求，返回旅行计划。前端是用Vue3和TypeScript实现的，是一个单页应用(SPA)，用户在浏览器中填写表单，点击"开始规划"按钮，前端发送HTTP请求到后端，等待响应，然后渲染结果页面。整个过程中，页面不会刷新，用户体验很流畅。
...
<table><tr><td>技术</td><td>用途</td></tr><tr><td>Vue 3</td><td>前端框架(组合式API)</td></tr><tr><td>JavaScript</td><td>类型安全</td></tr><tr><td>Vite</td><td>构建工具</td></tr><tr><td>Ant Design Vue</td><td>UI组件库</td></tr><tr><td>Axios</td><td>HTTP客户端</td></tr><tr><td>Vue Router</td><td>路由管理</td></tr><tr><td>@amap/amate-jsapi-loader</td><td>高德地图加载器</td></tr><tr><td>html2canvas</td><td>截图导出</td></tr><tr><td>jspdf</td><td>PDF导出</td></tr></table>
...
其中views目录存放页面组件，services目录存放API调用逻辑，types目录存放JavaScript类型定义，router目录存放路由配置。
...
const api $=$ axis.create({ baseURL:'http://localhost:8000/api', timeout:120000，//2分钟超时 headers:{ 'Content-Type':'application/json' ）   
}）
...
```javascript
//请求拦截器  
api.interceptors.request.use(config => {console.log('发送请求：',config)return config},error => Promisereject(error))  
//响应拦截器  
api.interceptors(response.use(response => {console.log('收到响应：',response)return response},error => {console.error('请求失败：',error)return Promisereject(error)}）
```
...
注意这个函数的类型签名：参数是TripPlanRequest类型，返回值是Promise<Triplan>类型。这意味着JavaScript会检查调用者传递的参数是否符合要求，也会检查返回值的使用是否正确。

# 13.5.4 Home 表单设计

Home页面是用户的入口，包含一个表单，让用户填写旅行需求。我们使用Vue3的CompositionAPI来组织代码：

```typescript
<script setup lang="ts"> import { ref } from 'vue' import { useRouter } from 'vue-router' import { message } from 'ant-design-vue' import { generateTripPlan } from '@services/api' import type { TripPlanRequest } from '@types' const router = useRouter() const loading = ref(false) 
```

```vue
const loadingProgress = ref(0)  
const loadingStatus = ref('')  
const formData = ref< tripsPlanRequest>({}  
    city: '',  
    start_date: '',  
    end_date: '',  
    days: 3,  
...
const handleSubmit $=$ async $\mathbf{\Phi}()\Rightarrow$ { loading.value $=$ true loadingProgress.value $= 0$ //模拟进度更新   
constprogressInterval $=$ setInterval( $\mathbf{\Phi}(.)\Rightarrow$ { if (loadingProgress.value $<  90$ ）{ loadingProgress.value $+ = 10$ if (loadingProgress.value $<  30$ ) loadingstatus.value $=$ '正在搜索景点...’ else if (loadingProgress.value $<  50$ ) loadingstatus.value $=$ 正在查询天气...’ else if (loadingProgress.value $<  70$ ) loadingstatus.value $=$ 正在推荐酒店...’ else loadingstatus.value $=$ 正在生成行程计划...’ } },500)   
try{ const response $=$ await generateTripPlan(formData.value) clearInterval(progressInterval) loadingProgress.value $= 100$ router.push({name:'result',state:{tripPlan:response}}） } catch (error){ clearInterval(progressInterval) message(error('生成计划失败，请重试')) }finally{ loading.value $=$ false }
...
```vue
<template> <div class="home-container"> <div class="page-header"> <h1 class="page-title"> 智能旅行助手</h1> <p class="page-subtitle">基于AI的个性化旅行规划</p> </div> <a-card class="form-card"> <a-form :model="formData" @finish="handleSubmit"> <a-form-item label="目的地城市" name="city":rules=[[{required: true}]]"> <a-input v-model:value="formData.city" placeholder="如：北京" /> </a-form-item> <!-- 更多表单项... --> <a-form-item> <a-button type="primary" html-type="submit" size="large" :loading="loading"> 开始规划 </a-button> </a-form-item> <!-- 加载进度条 --> <a-form-item v-if="loading"> <a-progress :percent="loadingProgress" status="active" /> <p>{loadingStatus} </p> </a-form-item> </a-form> </a-card> </div> </template>
```
...
```javascript
const AMap = await AMapLoader.load({ key: 'your_amap_web_key', version: '2.0' })  
map = new AMap.Map('amap-container',[zoom: 12, center: [116.397128, 39.916527] ])  
// 添加景点标记  
tripPlan.value.days.forEach(day) => { day.atTRAactions.forEach((attraction,index) => { const marker = new AMap.Marker({ position: [attraction.location.longitude,attraction.locationlatitude], title: attraction.name, label:{content:'{\index + 1}',direction:'top'} }) map.addmarker)}  
}
```

这段代码首先加载高德地图 SDK，然后创建地图实例，最后遍历所有景点，为每个景点创建一个标记 (Marker)。标记的位置是景点的经纬度坐标，这些坐标是从后端的 Attraction 对象中获取的。

导出功能使用html2canvas和jsPDF库。html2canvas可以把DOM元素转换成Canvas，然后我们可以把Canvas导出为图片或PDF：
...
const exportAsPDF = async() => {  
    const element = document.getElementById('trip-plan-content')  
    const canvas = await html2canvas.element, { scale: 2 } )  
    const imgData = canvastoaURL('image/png')  
...
pdf.save({ $\S$ {tripPlan.value.city}旅行计划.pdf}
...
```txt
<a-row :gutter="16"> <a-col :span="6"> <a-statistic title="景点门票" :value="tripPlan.budget.total(attractions" suffix="元" /> </a-col> <a-col :span="6"> <a-statistic title="酒店住宿" :value="tripPlan.budget.total_hoteiels" suffix="元" /> </a-col> <a-col :span="6"> <a-statistic title="餐饮费用" :value="tripPlan.budget.total.meals" suffix="元" /> </a-col> <a-col :span="6"> <a-statistic title="交通费用" :value="tripPlan.budget.total_transportation" suffix="元" /> </a-col> </a-row> <a-divider /> <a-row> <a-col :span="24" style="text-align:center;"> <a-statistic title="预估总费用" :value="tripPlan.budget.total" suffix="元" :value-style={'color:'#cf1322',fontsize:'32px',fontweight:'bold'" /> </a-col> </a-row> </a-card>
```
...
//进入编辑模式   
const toggleEditMode $\equiv$ () $\Rightarrow$ { editMode.value $=$ true originalPlan.value $=$ JSON.parse(JSON.stringify(tripPlan.value))   
}

注意这里使用了JSON.parse(JSON.stringify(...))来深拷贝对象。为什么不直接赋值？因为JavaScript中对象是引用类型，如果直接赋值，originalPlan和tripPlan会指向同一个对象，修改一个会影响另一个。深拷贝可以创建一个完全独立的副本。
...
const moveAttraction = (dayIndex: number,attractionIndex: number,direction:'up'| 'down') => {  
    ...
    if (newIndex >= 0 && newIndex < attractions.length) {  
        [attractions[attractionIndex],attractions[新加X] == [attractions[新加X],attractions[attractionIndex]]]  
    }  
}
```
...
const deleteAttraction = (dayIndex: number, attractionIndex: number) => {  
    tripPlan.value.days[dayIndex].attractions.splce(attractionIndex, 1)  
}
```
...
```javascript
// 保存修改  
const saveChanges = () => {editMode.value = falsemessage.success('修改已保存')initMap() //重新初始化地图  
}  
//取消编辑  
const cancelEdit = () => {if (originalPlan.value) {tripPlan.value = originalPlan.value}editMode.value = false}
```
...
1. 使用高德地图的静态地图 API：调用 maps(staticmap) 工具生成静态地图图片，替代动态地图
...
const link = document.createElement('a')  
link.download = ${tripPlan.value.city}旅行计划.png  
linkhref = canvastoaURL('image/png')  
link.click()  
...
import jsPDF from 'jspdf'   
const exportAsPDF $=$ async( $\Rightarrow$ { //先截取地图 await captureMapImage() const element $=$ document.getElementById('trip-plan-content') if(!element）return const canvas $=$ await html2canvas.element,{ backgroundColor:'#FFFFFF', scale:2, useCORS:true, allowTaint:true }）   
//恢复地图 restoreMap() const pdf $=$ new jsPDF('p','mm','a4') const imgData $=$ canvas,toDataURL('image/png') const imgwidth $= 210$ //A4宽度 const imgHeight $=$ (canvas.height\*imgwidth)/canvas.width pdf.addImage(imgData,'PNG',0,0,imagewidth,imageHeight) pdf.save({ $\$ \}\{ \{ \text{游} \} \}\{ \text{游} \} \}\{ \text{游} \}

这里需要计算图片的高度，保持宽高比。A4 纸的宽度是 $210 \mathrm{~mm}$ ，我们根据 Canvas 的宽高比计算出对应的高度。
...
```html
<a-menu v-model:selectedKeys="[activeSection]" mode="inline" @click="scrollToSection"> <a-menu-item key="overview">行程概览</a-menu-item> <a-menu-item key="budget">预算明细</a-menu-item> <a-menu-item key="map">地图</a-menu-item> <a-menu-item key="days">每日行程</a-menu-item> <a-menu-item key="weather">天气</a-menu-item> </a-menu>
```
...
const scrollToSection = ({key}: {key: string}) => {  
    ...
    if (element) {  
        elementscrollIntoView({behavior: 'smooth', block: 'start'})}  
    }  
}
```
