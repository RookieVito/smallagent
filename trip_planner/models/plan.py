...
class Attraction(BaseModel):
    ...
    suggested_duration_minutes: int = Field(description="建议游览时长（分钟）", ge=1)
    ticket_price: Decimal | None = Field(default=None, description="门票价格（CNY），免费景点为 null")
...
class DailyForecast(BaseModel):
    ...
    condition: str = Field(description="天气状况，如 晴/多云/小雨/大雨")
...
class BudgetBreakdown(BaseModel):
    ...
    accommodation: Decimal = Field(description="住宿费用（CNY）")
    dining: Decimal = Field(description="餐饮费用（CNY）")
    attractions: Decimal = Field(description="景点门票费用（CNY）")
    transport: Decimal = Field(description="交通费用（CNY）")


class BudgetSummary(BaseModel):
    """预算汇总。"""

    estimated_total: Decimal = Field(description="预估总费用（CNY）")
...
class TripPlan(BaseModel):
    ...
    created_at: datetime | None = Field(default=None, description="计划创建时间（ISO 8601），由服务端生成")
